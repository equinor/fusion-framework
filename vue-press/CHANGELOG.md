...
- 18ee1cb: Add the analytics module's `docs/` pages (`adapters.md`, `collectors.md`, `tracking-events.md`,
...
- 18ee1cb: Add the missing `telemetry` row (`TelemetryMockConfigurator`) to the accessor table in the framework's testing.md guide, left out when the telemetry mock was originally documented.
- 18ee1cb: Add a "Testing" page to the HTTP module's vue-press docs, `@include`ing the package's own `docs/testing.md`, and add the corresponding sidebar entry.
- bd43eca: Add the OpenAPI mock server reference covering executable `defineService` modules, service
...
- 18ee1cb: Update React Router interop test examples to use the supported Vitest browser renderer.
- 18ee1cb: Add a journey-oriented guide for testing Fusion React apps with Vitest, including browser setup, app fixtures, deterministic platform mocks, advanced composition, and links to maintained module references and runnable cookbooks.
...
- 09a9bcc: Add the OpenAPI mock server reference covering executable `defineService` modules, service
  discovery modes, local development overlays, Playwright overrides, reset behavior, and the
  `ffc app dev --mock` workflow. Include a migration guide that maps service mocks in
  `dev-server.config.ts` to `serviceDiscovery`, `routes`, `middleware`, and `components`, while
  identifying host-level configuration that should remain in the dev server. Also add missing
...
- 1cab989: chore: bump mermaid from 11.12.3 to 11.13.0
...
- abffa53: Add React Router module documentation page to vue-press with sidebar navigation.
...
- 8f30948: Add readme to service discovery doc in VuePress
- abffa53: Refresh the HTTP module documentation to match the current public API, including client creation, MSAL scope behavior, selectors, operators, and SSE usage.
- abffa53: Refresh module documentation for React Router and HTTP modules.
  - Add React Router module documentation page to vue-press with sidebar navigation
  - Improve Fusion React Router README with quick start, route DSL overview, Fusion context usage, and route schema examples
  - Refresh HTTP module README to match current public API including client creation, MSAL scopes, selectors, operators, and SSE usage

- abffa53: Improve Fusion React Router README with a clearer quick start, route DSL overview, Fusion context usage, and route schema examples to make it easier for application developers to adopt the extended router.
