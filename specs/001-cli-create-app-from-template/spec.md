...
1. **Given** a developer wants to create a new app, **When** they run `fusion create app my-new-app`, **Then** the system creates a new application directory with all necessary files and dependencies
2. **Given** a developer wants to choose a specific template, **When** they run `fusion create app my-app --template app-react`, **Then** the system creates an app using only the specified template
3. **Given** a developer wants to skip interactive prompts, **When** they run `fusion create app my-app --default`, **Then** the system creates an app using default options without user interaction
...
- **FR-001**: System MUST provide a `fusion create app <app-name>` command that creates new Fusion Framework applications
...
- **FR-004**: System MUST support `--template` flag to specify a particular template
- **FR-005**: System MUST support `--default` flag to skip interactive prompts and use default options
- **FR-006**: System MUST support `--debug` flag for verbose output during creation
- **FR-007**: System MUST initialize a git repository in the created application directory
- **FR-008**: System MUST ask user whether to install dependencies after creating the application, defaulting to yes
- **FR-009**: System MUST ask user whether to start the development server after dependency installation, defaulting to no
- **FR-010**: System MUST ask user whether to open the created project in the developer's IDE, defaulting to no
- **FR-011**: System MUST ask user whether to clean up temporary files after successful creation, defaulting to no cleanup
- **FR-012**: System MUST validate that the app name is valid and doesn't conflict with existing directories
- **FR-013**: System MUST provide clear error messages when template cloning fails
- **FR-014**: System MUST provide clear error messages when dependency installation fails
- **FR-015**: System MUST support both `npx @equinor/fusion-framework-cli` and local `ffc` command usage
- **FR-016**: System MUST support `--help` flag to display usage information and available options
