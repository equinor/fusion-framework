...
- **`filename-convention`**: rename the file to match its sole value export (kebab-case for functions/consts, PascalCase for classes). Splitting a multi-export file can surface NEW filename-convention warnings on the resulting files that weren't visible before the split — always re-lint after any split. **Never rename a file named `static.ts` or `types.ts`** — both are in this rule's `excludePattern` in `fusion-lint.config.json` by repo convention: `static.ts` holds static/constant exports regardless of the constant's name, and `types.ts` may legitimately hold a single `enum` alongside many type-only exports. If fusion-lint still flags one of these basenames, fix the config's `excludePattern`, don't rename the file.
- **`single-export-per-file`**: only `index.ts` is exempt by default (plus whatever else is in `fusion-lint.config.json`'s `excludePattern`, e.g. `static.ts`, `module.ts`, `*-module.ts`, `*.schemas.ts`). Extract every export past the first into its own file and update all importers. Type-only exports AND `enum` declarations don't count toward the limit — enums are allowed to live alongside their related types.
