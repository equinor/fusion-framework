...
import { ApiAppServiceNowConfigurationSchemaV1 } from './api-app-service-now-configuration-schema-v1';
...
/**
 * Zod schema for the `ApiApp` model published by the Fusion Apps API 1.0.
 *
 * Detailed representation of a Fusion application, including build version, category, contexts,
 * admins, owners, and data classification.
 */
export const ApiAppSchemaV1 = z
  .object({
    ...
    /** Counts of the compliance findings currently open for the app. */
    ...
    /** ServiceNow support configuration of the application. */
    serviceNowConfiguration: ApiAppServiceNowConfigurationSchemaV1.nullish().describe(
      'ServiceNow support configuration of the application.',
    ),
  ...
  .describe(
    'Detailed representation of a Fusion application, including build version, category, contexts, admins, owners, and data classification.',
  );
 ...
 * admins, owners, and data classification.
 *
 * Apps API 1.0 model inferred from {@link ApiAppSchemaV1}, so `ApiAppV1` and the runtime validator
 * can never describe different shapes.
 */
export type ApiAppV1 = z.infer<typeof ApiAppSchemaV1>;
