...
/**
 * Builds a schema that faithfully validates the paged-collection envelope Apps API 1.0 wraps
 * around every collection resource, while delegating item validation to the provided
 * `itemSchema`.
 *
 * The envelope is a factory rather than a fixed schema because the item type differs per
 * endpoint; {@link ApiPagedCollectionV1} infers the resulting shape, so the envelope is still
 ...
 * @example
 * ```ts
 * const pageSchema = apiPagedCollectionSchemaV1(ApiAppListItemSchemaV1);
...
export const apiPagedCollectionSchemaV1 = <TItem>(itemSchema: z.ZodType<TItem>) =>
  z
    ...
    .describe(
      'The paged-collection envelope Apps API 1.0 wraps around every collection resource, while delegating item validation to the provided `itemSchema`.',
    );

/**
 * One page of an Apps API 1.0 collection: the `value` array plus the `totalCount`, `count`,
