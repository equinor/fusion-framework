...
/**
 * API version identifiers `listMyApps` accepts for `GET /persons/me/apps`.
 *
 * Fusion Apps publishes version 1.0 of this operation, nameable as `'v1'`, `'1.0'`, or
 * `ApiVersion.v1`. All three resolve to the same contract entry, so all three infer the same
 * {@link ListMyAppsArg} and {@link ListMyAppsResponse}.
 */
type ListMyAppsVersion = FilterAllowedApiVersions<AvailableVersions>;

/**
 * Version contract for `listMyApps`, implementing `GET /persons/me/apps`.
 *
...
const VersionContract = {
  [ApiVersion.v1]: {
    ...
    args: z.object({}).describe('Arguments for GET /persons/me/apps (listMyApps v1.0).'),
...
/**
 * Response body `listMyApps` resolves for the selected API version.
 *
 ...
 */
type ListMyAppsResponse<TVersion extends ListMyAppsVersion> = VersionedResponse<
  typeof VersionContract,
  ExtractApiVersion<TVersion>
>;

/**
 * Arguments `listMyApps` accepts, resolved from the selected API version.
 *
 ...
 */
type ListMyAppsArg<TVersion extends ListMyAppsVersion> = VersionedArgs<
  typeof VersionContract,
  ExtractApiVersion<TVersion>
>;

/**
 * What `listMyApps` hands back once the request runs, for the selected API version and client
 * method.
 *
 * `'json'` gives `Promise<ListMyAppsResponse<TVersion>>` and `'json$'` gives
 * `StreamResponse<ListMyAppsResponse<TVersion>>`, so promise and observable callers share one
 * response type.
 */
type ListMyAppsResult<
  TVersion extends ListMyAppsVersion,
  TMethod extends ClientMethodType = 'json',
> = ClientMethod<ListMyAppsResponse<TVersion>>[TMethod];
...
const generateRequestParameters = <TResult, TVersion extends AvailableVersions>(
  ...
  switch (version) {
    case ApiVersion.v1: {
      const baseInit: FetchRequestInit<ListMyAppsResponse<ApiVersion.v1>, JsonRequest> = {
...
/**
 * Lists the apps available to the signed-in person.
 *
 * Fusion Apps API operation: `GET /persons/me/apps` — "Gets all apps for person."
 *
 * Curried in two stages: `listMyApps(version, client, method)` binds the API version, the
 * `IHttpClient` that reaches the Apps service, and the execution method — `'json'` for a promise,
 * `'json$'` for an observable stream. The returned function takes {@link ListMyAppsArg} plus an
 * optional `ClientRequestInit`, and gives back {@link ListMyAppsResult}.
 *
 ...
 *
 * Related: `getMyApp`, `listMyPinnedApps`.
 *
 ...
 * @returns A request function taking {@link ListMyAppsArg} and an optional
 * `ClientRequestInit`, returning {@link ListMyAppsResult}.
 ...
 * @example
 * ```ts
 * import { listMyApps } from '@equinor/fusion-services/apps';
 *
 * const result = await listMyApps('v1', httpClient)();
 * ```
 */
const listMyApps = <TVersion extends ListMyAppsVersion, TMethod extends ClientMethodType = 'json'>(
  version: TVersion,
  client: IHttpClient,
  ...
  return (
    input?: ListMyAppsArg<MethodVersion>,
    init?: ClientRequestInit<IHttpClient, ListMyAppsResponse<MethodVersion>>,
  ): ListMyAppsResult<MethodVersion, TMethod> => {
    // The operation has no required arguments, so an omitted argument object parses as empty.
    const args = parseVersionedArgs(VersionContract, apiVersion, input ?? {});
    return client[method](
      generateApiPath(apiVersion, args),
      generateRequestParameters(apiVersion, args, init),
    ) as ListMyAppsResult<MethodVersion, TMethod>;
  };
};
