...
/**
 * API version identifiers `deleteTechnologyProduct` accepts for `DELETE
 ...
 * {@link DeleteTechnologyProductArg} and {@link DeleteTechnologyProductResponse}.
 */
type DeleteTechnologyProductVersion = FilterAllowedApiVersions<AvailableVersions>;

/**
 * Version contract for `deleteTechnologyProduct`, implementing `DELETE
 * /libraries/technology-products/{technologyProductIdentifier}`.
 *
...
const VersionContract = {
  [ApiVersion.v1]: {
    ...
    args: z
      ...
      .describe(
        'Arguments for DELETE /libraries/technology-products/{technologyProductIdentifier} (deleteTechnologyProduct v1.0).',
      ),
...
/**
 * Response body `deleteTechnologyProduct` resolves for the selected API version.
 *
 * Version 1.0 answers `204 No Content` without a body, so the type resolves to `void`.
 */
type DeleteTechnologyProductResponse<TVersion extends DeleteTechnologyProductVersion> =
  VersionedResponse<typeof VersionContract, ExtractApiVersion<TVersion>>;

/**
 * Arguments `deleteTechnologyProduct` accepts, resolved from the selected API version.
 *
 ...
 */
type DeleteTechnologyProductArg<TVersion extends DeleteTechnologyProductVersion> = VersionedArgs<
  typeof VersionContract,
  ExtractApiVersion<TVersion>
>;

/**
 * What `deleteTechnologyProduct` hands back once the request runs, for the selected API version and
 * client method.
 *
 * `'json'` gives `Promise<DeleteTechnologyProductResponse<TVersion>>` and `'json$'` gives
 * `StreamResponse<DeleteTechnologyProductResponse<TVersion>>`, so promise and observable callers
 * share one response type.
 */
type DeleteTechnologyProductResult<
  TVersion extends DeleteTechnologyProductVersion,
  TMethod extends ClientMethodType = 'json',
> = ClientMethod<DeleteTechnologyProductResponse<TVersion>>[TMethod];
  ...
  switch (version) {
    case ApiVersion.v1: {
      const baseInit: FetchRequestInit<
        DeleteTechnologyProductResponse<ApiVersion.v1>,
        JsonRequest
      > = {
        method: 'DELETE',
...
/**
 * Delete a specific technology product.
 *
 * Fusion Apps API operation: `DELETE /libraries/technology-products/{technologyProductIdentifier}`
 * — "Delete a specific technology product."
 *
 * Curried in two stages: `deleteTechnologyProduct(version, client, method)` binds the API version,
 ...
 * {@link DeleteTechnologyProductArg} plus an optional `ClientRequestInit`, and gives back
 * {@link DeleteTechnologyProductResult}.
 *
 ...
 * @returns A request function taking {@link DeleteTechnologyProductArg} and an optional
 * `ClientRequestInit`, returning {@link DeleteTechnologyProductResult}.
 ...
 * @example
 * ```ts
 * import { deleteTechnologyProduct } from '@equinor/fusion-services/apps';
 *
 * await deleteTechnologyProduct('v1', httpClient)({
 *   technologyProductIdentifier: 'fusion-core',
 * });
 * ```
 */
const deleteTechnologyProduct = <
  TVersion extends DeleteTechnologyProductVersion,
  TMethod extends ClientMethodType = 'json',
>(
  version: TVersion,
  ...
  return (
    input: DeleteTechnologyProductArg<MethodVersion>,
    init?: ClientRequestInit<IHttpClient, DeleteTechnologyProductResponse<MethodVersion>>,
  ): DeleteTechnologyProductResult<MethodVersion, TMethod> => {
    const args = parseVersionedArgs(VersionContract, apiVersion, input);
    return client[method](
      generateApiPath(apiVersion, args),
      generateRequestParameters(apiVersion, args, init),
    ) as DeleteTechnologyProductResult<MethodVersion, TMethod>;
  };
};
