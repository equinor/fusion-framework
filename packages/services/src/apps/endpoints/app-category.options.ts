...
/**
 * API version identifiers `checkAppCategoryAccess` accepts for `OPTIONS
 ...
 * {@link CheckAppCategoryAccessArg} and {@link CheckAppCategoryAccessResponse}.
 */
type CheckAppCategoryAccessVersion = FilterAllowedApiVersions<AvailableVersions>;

/**
 * Version contract for `checkAppCategoryAccess`, implementing `OPTIONS
 * /apps/categories/{appCategoryIdentifier}`.
 *
...
const VersionContract = {
  [ApiVersion.v1]: {
    ...
    args: z
      ...
      .describe(
        'Arguments for OPTIONS /apps/categories/{appCategoryIdentifier} (checkAppCategoryAccess v1.0).',
      ),
...
/**
 * Response body `checkAppCategoryAccess` resolves for the selected API version.
 *
 * Version 1.0 answers `204 No Content` without a body, so the type resolves to `void`.
 */
type CheckAppCategoryAccessResponse<TVersion extends CheckAppCategoryAccessVersion> =
  VersionedResponse<typeof VersionContract, ExtractApiVersion<TVersion>>;

/**
 * Arguments `checkAppCategoryAccess` accepts, resolved from the selected API version.
 *
 * Version 1.0 accepts the path identifier `appCategoryIdentifier`.
 *
 ...
 */
type CheckAppCategoryAccessArg<TVersion extends CheckAppCategoryAccessVersion> = VersionedArgs<
  typeof VersionContract,
  ExtractApiVersion<TVersion>
>;

/**
 * What `checkAppCategoryAccess` hands back once the request runs, for the selected API version and
 * client method.
 *
 * `'json'` gives `Promise<CheckAppCategoryAccessResponse<TVersion>>` and `'json$'` gives
 * `StreamResponse<CheckAppCategoryAccessResponse<TVersion>>`, so promise and observable callers
 * share one response type.
 */
type CheckAppCategoryAccessResult<
  TVersion extends CheckAppCategoryAccessVersion,
  TMethod extends ClientMethodType = 'json',
> = ClientMethod<CheckAppCategoryAccessResponse<TVersion>>[TMethod];
  ...
  switch (version) {
    case ApiVersion.v1: {
      const baseInit: FetchRequestInit<
        CheckAppCategoryAccessResponse<ApiVersion.v1>,
        JsonRequest
      > = {
        method: 'OPTIONS',
...
/**
 * Endpoint that returns a response header with allowed http methods for the app category.
 *
 * Fusion Apps API operation: `OPTIONS /apps/categories/{appCategoryIdentifier}` — "Endpoint that
 * returns a response header with allowed http methods for the app category."
 *
 * Curried in two stages: `checkAppCategoryAccess(version, client, method)` binds the API version,
 ...
 * {@link CheckAppCategoryAccessArg} plus an optional `ClientRequestInit`, and gives back
 * {@link CheckAppCategoryAccessResult}.
 *
 ...
 * @returns A request function taking {@link CheckAppCategoryAccessArg} and an optional
 * `ClientRequestInit`, returning {@link CheckAppCategoryAccessResult}.
 ...
 * @example
 * ```ts
 * import { checkAppCategoryAccess } from '@equinor/fusion-services/apps';
 *
 * await checkAppCategoryAccess('v1', httpClient)({ appCategoryIdentifier: 'productivity' });
 * ```
 */
const checkAppCategoryAccess = <
  TVersion extends CheckAppCategoryAccessVersion,
  TMethod extends ClientMethodType = 'json',
>(
  version: TVersion,
  ...
  return (
    input: CheckAppCategoryAccessArg<MethodVersion>,
    init?: ClientRequestInit<IHttpClient, CheckAppCategoryAccessResponse<MethodVersion>>,
  ): CheckAppCategoryAccessResult<MethodVersion, TMethod> => {
    const args = parseVersionedArgs(VersionContract, apiVersion, input);
    return client[method](
      generateApiPath(apiVersion, args),
      generateRequestParameters(apiVersion, args, init),
    ) as CheckAppCategoryAccessResult<MethodVersion, TMethod>;
  };
};
