...
/**
 * API version identifiers `checkContextTypesAccess` accepts for `OPTIONS /context-types`.
 *
 * Fusion Apps publishes version 1.0 of this operation, nameable as `'v1'`, `'1.0'`, or
 * `ApiVersion.v1`. All three resolve to the same contract entry, so all three infer the same
 * {@link CheckContextTypesAccessArg} and {@link CheckContextTypesAccessResponse}.
 */
type CheckContextTypesAccessVersion = FilterAllowedApiVersions<AvailableVersions>;

/**
 * Version contract for `checkContextTypesAccess`, implementing `OPTIONS /context-types`.
 *
...
const VersionContract = {
  [ApiVersion.v1]: {
    ...
    args: z
      .object({})
      .describe('Arguments for OPTIONS /context-types (checkContextTypesAccess v1.0).'),
...
/**
 * Response body `checkContextTypesAccess` resolves for the selected API version.
 *
 * Version 1.0 answers `204 No Content` without a body, so the type resolves to `void`.
 */
type CheckContextTypesAccessResponse<TVersion extends CheckContextTypesAccessVersion> =
  VersionedResponse<typeof VersionContract, ExtractApiVersion<TVersion>>;

/**
 * Arguments `checkContextTypesAccess` accepts, resolved from the selected API version.
 *
 ...
 */
type CheckContextTypesAccessArg<TVersion extends CheckContextTypesAccessVersion> = VersionedArgs<
  typeof VersionContract,
  ExtractApiVersion<TVersion>
>;

/**
 * What `checkContextTypesAccess` hands back once the request runs, for the selected API version and
 * client method.
 *
 * `'json'` gives `Promise<CheckContextTypesAccessResponse<TVersion>>` and `'json$'` gives
 * `StreamResponse<CheckContextTypesAccessResponse<TVersion>>`, so promise and observable callers
 * share one response type.
 */
type CheckContextTypesAccessResult<
  TVersion extends CheckContextTypesAccessVersion,
  TMethod extends ClientMethodType = 'json',
> = ClientMethod<CheckContextTypesAccessResponse<TVersion>>[TMethod];
  ...
  switch (version) {
    case ApiVersion.v1: {
      const baseInit: FetchRequestInit<
        CheckContextTypesAccessResponse<ApiVersion.v1>,
        JsonRequest
      > = {
        method: 'OPTIONS',
...
/**
 * Endpoint that returns a response header with allowed http methods for context types.
 *
 * Fusion Apps API operation: `OPTIONS /context-types` — "Endpoint that returns a response header
 * with allowed http methods for context types."
 *
 * Curried in two stages: `checkContextTypesAccess(version, client, method)` binds the API version,
 ...
 * {@link CheckContextTypesAccessArg} plus an optional `ClientRequestInit`, and gives back
 * {@link CheckContextTypesAccessResult}.
 *
 ...
 * @returns A request function taking {@link CheckContextTypesAccessArg} and an optional
 * `ClientRequestInit`, returning {@link CheckContextTypesAccessResult}.
 ...
 * @example
 * ```ts
 * import { checkContextTypesAccess } from '@equinor/fusion-services/apps';
 *
 * await checkContextTypesAccess('v1', httpClient)();
 * ```
 */
const checkContextTypesAccess = <
  TVersion extends CheckContextTypesAccessVersion,
  TMethod extends ClientMethodType = 'json',
>(
  version: TVersion,
  ...
  return (
    input?: CheckContextTypesAccessArg<MethodVersion>,
    init?: ClientRequestInit<IHttpClient, CheckContextTypesAccessResponse<MethodVersion>>,
  ): CheckContextTypesAccessResult<MethodVersion, TMethod> => {
    // The operation has no required arguments, so an omitted argument object parses as empty.
    const args = parseVersionedArgs(VersionContract, apiVersion, input ?? {});
    return client[method](
      generateApiPath(apiVersion, args),
      generateRequestParameters(apiVersion, args, init),
    ) as CheckContextTypesAccessResult<MethodVersion, TMethod>;
  };
};
