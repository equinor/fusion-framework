...
/**
 * API version identifiers `checkAppGovernanceDocumentsAccess` accepts for `OPTIONS
 ...
 * {@link CheckAppGovernanceDocumentsAccessArg} and
 * {@link CheckAppGovernanceDocumentsAccessResponse}.
 */
type CheckAppGovernanceDocumentsAccessVersion = FilterAllowedApiVersions<AvailableVersions>;

/**
 * Version contract for `checkAppGovernanceDocumentsAccess`, implementing `OPTIONS
 * /apps/{appIdentifier}/governance/documents`.
 *
...
const VersionContract = {
  [ApiVersion.v1]: {
    ...
    args: z
      ...
      .describe(
        'Arguments for OPTIONS /apps/{appIdentifier}/governance/documents (checkAppGovernanceDocumentsAccess v1.0).',
      ),
...
/**
 * Response body `checkAppGovernanceDocumentsAccess` resolves for the selected API version.
 *
...
type CheckAppGovernanceDocumentsAccessResponse<
  TVersion extends CheckAppGovernanceDocumentsAccessVersion,
> = VersionedResponse<typeof VersionContract, ExtractApiVersion<TVersion>>;

/**
 * Arguments `checkAppGovernanceDocumentsAccess` accepts, resolved from the selected API version.
 *
...
/**
 * What `checkAppGovernanceDocumentsAccess` hands back once the request runs, for the selected API
 * version and client method.
 *
 * `'json'` gives `Promise<CheckAppGovernanceDocumentsAccessResponse<TVersion>>` and `'json$'` gives
 * `StreamResponse<CheckAppGovernanceDocumentsAccessResponse<TVersion>>`, so promise and observable
 * callers share one response type.
 */
type CheckAppGovernanceDocumentsAccessResult<
  TVersion extends CheckAppGovernanceDocumentsAccessVersion,
  TMethod extends ClientMethodType = 'json',
> = ClientMethod<CheckAppGovernanceDocumentsAccessResponse<TVersion>>[TMethod];
  ...
  switch (version) {
    case ApiVersion.v1: {
      const baseInit: FetchRequestInit<
        CheckAppGovernanceDocumentsAccessResponse<ApiVersion.v1>,
        JsonRequest
      > = {
        method: 'OPTIONS',
...
/**
 * Endpoint that returns a response header with allowed http methods governance documents for the
 * specified app.
 *
 * Fusion Apps API operation: `OPTIONS /apps/{appIdentifier}/governance/documents` — "Endpoint that
 * returns a response header with allowed http methods governance documents for the specified app."
 *
 * Curried in two stages: `checkAppGovernanceDocumentsAccess(version, client, method)` binds the API
 ...
 * {@link CheckAppGovernanceDocumentsAccessArg} plus an optional `ClientRequestInit`, and gives back
 * {@link CheckAppGovernanceDocumentsAccessResult}.
 *
 ...
 * @returns A request function taking {@link CheckAppGovernanceDocumentsAccessArg} and an optional
 * `ClientRequestInit`, returning {@link CheckAppGovernanceDocumentsAccessResult}.
 ...
 * @example
 * ```ts
 * import { checkAppGovernanceDocumentsAccess } from '@equinor/fusion-services/apps';
 *
 * await checkAppGovernanceDocumentsAccess('v1', httpClient)({ appIdentifier: 'my-app' });
 * ```
 */
const checkAppGovernanceDocumentsAccess = <
  TVersion extends CheckAppGovernanceDocumentsAccessVersion,
  TMethod extends ClientMethodType = 'json',
>(
  version: TVersion,
  ...
  return (
    input: CheckAppGovernanceDocumentsAccessArg<MethodVersion>,
    init?: ClientRequestInit<IHttpClient, CheckAppGovernanceDocumentsAccessResponse<MethodVersion>>,
  ): CheckAppGovernanceDocumentsAccessResult<MethodVersion, TMethod> => {
    const args = parseVersionedArgs(VersionContract, apiVersion, input);
    return client[method](
      generateApiPath(apiVersion, args),
      generateRequestParameters(apiVersion, args, init),
    ) as CheckAppGovernanceDocumentsAccessResult<MethodVersion, TMethod>;
  };
};
