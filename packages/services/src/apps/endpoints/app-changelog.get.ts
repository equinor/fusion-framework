...
/**
 * API version identifiers `listAppChangelog` accepts for `GET /apps/{appIdentifier}/changelog`.
 *
 * Fusion Apps publishes version 1.0 of this operation, nameable as `'v1'`, `'1.0'`, or
 * `ApiVersion.v1`. All three resolve to the same contract entry, so all three infer the same
 * {@link ListAppChangelogArg} and {@link ListAppChangelogResponse}.
 */
type ListAppChangelogVersion = FilterAllowedApiVersions<AvailableVersions>;

/**
 * Version contract for `listAppChangelog`, implementing `GET /apps/{appIdentifier}/changelog`.
 *
...
const VersionContract = {
  [ApiVersion.v1]: {
    ...
    args: z
      ...
      .describe('Arguments for GET /apps/{appIdentifier}/changelog (listAppChangelog v1.0).'),
...
/**
 * Response body `listAppChangelog` resolves for the selected API version.
 *
 ...
 * body at runtime.
 */
type ListAppChangelogResponse<TVersion extends ListAppChangelogVersion> = VersionedResponse<
  typeof VersionContract,
  ExtractApiVersion<TVersion>
>;

/**
 * Arguments `listAppChangelog` accepts, resolved from the selected API version.
 *
 * Version 1.0 accepts the path identifier `appIdentifier`, and the optional query options `filter`,
 * `orderBy`, `top`, `skip`.
 *
 ...
 */
type ListAppChangelogArg<TVersion extends ListAppChangelogVersion> = VersionedArgs<
  typeof VersionContract,
  ExtractApiVersion<TVersion>
>;

/**
 * What `listAppChangelog` hands back once the request runs, for the selected API version and client
 * method.
 *
 * `'json'` gives `Promise<ListAppChangelogResponse<TVersion>>` and `'json$'` gives
 * `StreamResponse<ListAppChangelogResponse<TVersion>>`, so promise and observable callers share one
 * response type.
 */
type ListAppChangelogResult<
  TVersion extends ListAppChangelogVersion,
  TMethod extends ClientMethodType = 'json',
> = ClientMethod<ListAppChangelogResponse<TVersion>>[TMethod];
...
const generateRequestParameters = <TResult, TVersion extends AvailableVersions>(
  ...
  switch (version) {
    case ApiVersion.v1: {
      const baseInit: FetchRequestInit<ListAppChangelogResponse<ApiVersion.v1>, JsonRequest> = {
...
/**
 * Lists the changelog entries recorded for an app.
 *
 * Fusion Apps API operation: `GET /apps/{appIdentifier}/changelog`
 *
 * Curried in two stages: `listAppChangelog(version, client, method)` binds the API version, the
 * `IHttpClient` that reaches the Apps service, and the execution method — `'json'` for a promise,
 * `'json$'` for an observable stream. The returned function takes {@link ListAppChangelogArg} plus
 * an optional `ClientRequestInit`, and gives back {@link ListAppChangelogResult}.
 *
 ...
 * operation.
 *
 * Related: `listChangelog`, `listAppBuildChangelog`.
 *
 ...
 * @returns A request function taking {@link ListAppChangelogArg} and an optional
 * `ClientRequestInit`, returning {@link ListAppChangelogResult}.
 ...
 * @example
 * ```ts
 * import { listAppChangelog } from '@equinor/fusion-services/apps';
 *
 * const result = await listAppChangelog('v1', httpClient)({
 *   appIdentifier: 'my-app',
 *   filter: 'name eq 1',
 * });
 * ```
 */
const listAppChangelog = <
  TVersion extends ListAppChangelogVersion,
  TMethod extends ClientMethodType = 'json',
>(
  version: TVersion,
  ...
  return (
    input: ListAppChangelogArg<MethodVersion>,
    init?: ClientRequestInit<IHttpClient, ListAppChangelogResponse<MethodVersion>>,
  ): ListAppChangelogResult<MethodVersion, TMethod> => {
    const args = parseVersionedArgs(VersionContract, apiVersion, input);
    return client[method](
      generateApiPath(apiVersion, args),
      generateRequestParameters(apiVersion, args, init),
    ) as ListAppChangelogResult<MethodVersion, TMethod>;
  };
};
