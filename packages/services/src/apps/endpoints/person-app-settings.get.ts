...
/**
 * API version identifiers `getPersonAppSettings` accepts for `GET
 ...
 * {@link GetPersonAppSettingsArg} and {@link GetPersonAppSettingsResponse}.
 */
type GetPersonAppSettingsVersion = FilterAllowedApiVersions<AvailableVersions>;

/**
 * Version contract for `getPersonAppSettings`, implementing `GET
 * /persons/{accountIdentifier}/apps/{appIdentifier}/settings`.
 *
...
const VersionContract = {
  [ApiVersion.v1]: {
    ...
    args: z
      ...
      .describe(
        'Arguments for GET /persons/{accountIdentifier}/apps/{appIdentifier}/settings (getPersonAppSettings v1.0).',
      ),
...
/**
 * Response body `getPersonAppSettings` resolves for the selected API version.
 *
 ...
 */
type GetPersonAppSettingsResponse<TVersion extends GetPersonAppSettingsVersion> = VersionedResponse<
  typeof VersionContract,
  ExtractApiVersion<TVersion>
>;

/**
 * Arguments `getPersonAppSettings` accepts, resolved from the selected API version.
 *
 ...
 */
type GetPersonAppSettingsArg<TVersion extends GetPersonAppSettingsVersion> = VersionedArgs<
  typeof VersionContract,
  ExtractApiVersion<TVersion>
>;

/**
 * What `getPersonAppSettings` hands back once the request runs, for the selected API version and
 * client method.
 *
 * `'json'` gives `Promise<GetPersonAppSettingsResponse<TVersion>>` and `'json$'` gives
 * `StreamResponse<GetPersonAppSettingsResponse<TVersion>>`, so promise and observable callers share
 * one response type.
 */
type GetPersonAppSettingsResult<
  TVersion extends GetPersonAppSettingsVersion,
  TMethod extends ClientMethodType = 'json',
> = ClientMethod<GetPersonAppSettingsResponse<TVersion>>[TMethod];
...
const generateRequestParameters = <TResult, TVersion extends AvailableVersions>(
  ...
  switch (version) {
    case ApiVersion.v1: {
      const baseInit: FetchRequestInit<GetPersonAppSettingsResponse<ApiVersion.v1>, JsonRequest> = {
...
/**
 * Fetches a person’s stored settings for one app.
 *
 * Fusion Apps API operation: `GET /persons/{accountIdentifier}/apps/{appIdentifier}/settings` —
 * "Retrieves settings for a specific app."
 *
 * Curried in two stages: `getPersonAppSettings(version, client, method)` binds the API version, the
 * `IHttpClient` that reaches the Apps service, and the execution method — `'json'` for a promise,
 * `'json$'` for an observable stream. The returned function takes {@link GetPersonAppSettingsArg}
 * plus an optional `ClientRequestInit`, and gives back {@link GetPersonAppSettingsResult}.
 *
 ...
 *
 * Related: `upsertPersonAppSettings`.
 *
 ...
 * @returns A request function taking {@link GetPersonAppSettingsArg} and an optional
 * `ClientRequestInit`, returning {@link GetPersonAppSettingsResult}.
 ...
 * @example
 * ```ts
 * import { getPersonAppSettings } from '@equinor/fusion-services/apps';
 *
 * const result = await getPersonAppSettings('v1', httpClient)({
 *   accountIdentifier: 'user@equinor.com',
 *   appIdentifier: 'my-app',
 * });
 * ```
 */
const getPersonAppSettings = <
  TVersion extends GetPersonAppSettingsVersion,
  TMethod extends ClientMethodType = 'json',
>(
  version: TVersion,
  ...
  return (
    input: GetPersonAppSettingsArg<MethodVersion>,
    init?: ClientRequestInit<IHttpClient, GetPersonAppSettingsResponse<MethodVersion>>,
  ): GetPersonAppSettingsResult<MethodVersion, TMethod> => {
    const args = parseVersionedArgs(VersionContract, apiVersion, input);
    return client[method](
      generateApiPath(apiVersion, args),
      generateRequestParameters(apiVersion, args, init),
    ) as GetPersonAppSettingsResult<MethodVersion, TMethod>;
  };
};
