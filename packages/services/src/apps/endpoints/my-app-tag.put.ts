...
/**
 * API version identifiers `setMyAppTag` accepts for `PUT /persons/me/apps/{appIdentifier}/tag`.
 *
 * Fusion Apps publishes version 1.0 of this operation, nameable as `'v1'`, `'1.0'`, or
 * `ApiVersion.v1`. All three resolve to the same contract entry, so all three infer the same
 * {@link SetMyAppTagArg} and {@link SetMyAppTagResponse}.
 */
type SetMyAppTagVersion = FilterAllowedApiVersions<AvailableVersions>;

/**
 * Version contract for `setMyAppTag`, implementing `PUT /persons/me/apps/{appIdentifier}/tag`.
 *
...
const VersionContract = {
  [ApiVersion.v1]: {
    ...
    args: z
      ...
      .describe('Arguments for PUT /persons/me/apps/{appIdentifier}/tag (setMyAppTag v1.0).'),
...
/**
 * Response body `setMyAppTag` resolves for the selected API version.
 *
 ...
 */
type SetMyAppTagResponse<TVersion extends SetMyAppTagVersion> = VersionedResponse<
  typeof VersionContract,
  ExtractApiVersion<TVersion>
>;

/**
 * Arguments `setMyAppTag` accepts, resolved from the selected API version.
 *
 * Version 1.0 accepts the path identifier `appIdentifier`, and the request body fields of
 * `CreatePersonAppTagRequestV1`. Path identifiers and body fields share one flat object; the
 ...
 */
type SetMyAppTagArg<TVersion extends SetMyAppTagVersion> = VersionedArgs<
  typeof VersionContract,
  ExtractApiVersion<TVersion>
>;

/**
 * What `setMyAppTag` hands back once the request runs, for the selected API version and client
 * method.
 *
 * `'json'` gives `Promise<SetMyAppTagResponse<TVersion>>` and `'json$'` gives
 * `StreamResponse<SetMyAppTagResponse<TVersion>>`, so promise and observable callers share one
 * response type.
 */
type SetMyAppTagResult<
  TVersion extends SetMyAppTagVersion,
  TMethod extends ClientMethodType = 'json',
> = ClientMethod<SetMyAppTagResponse<TVersion>>[TMethod];
...
const generateRequestParameters = <TResult, TVersion extends AvailableVersions>(
  ...
  switch (version) {
    case ApiVersion.v1: {
      ...
      const baseInit: FetchRequestInit<SetMyAppTagResponse<ApiVersion.v1>, JsonRequest> = {
        method: 'PUT',
...
/**
 * Overrides which app build the signed-in person receives by default.
 *
 * Fusion Apps API operation: `PUT /persons/me/apps/{appIdentifier}/tag` — "Use this endpoint to
 * override which app build person will get by default."
 *
 * Curried in two stages: `setMyAppTag(version, client, method)` binds the API version, the
 * `IHttpClient` that reaches the Apps service, and the execution method — `'json'` for a promise,
 * `'json$'` for an observable stream. The returned function takes {@link SetMyAppTagArg} plus an
 * optional `ClientRequestInit`, and gives back {@link SetMyAppTagResult}.
 *
 ...
 *
 * Related: `deleteMyAppTag`, `getMyApp`.
 *
 ...
 * @returns A request function taking {@link SetMyAppTagArg} and an optional
 * `ClientRequestInit`, returning {@link SetMyAppTagResult}.
 ...
 * @example
 * ```ts
 * import { setMyAppTag } from '@equinor/fusion-services/apps';
 *
 * const result = await setMyAppTag('v1', httpClient)({
...
>(
  ...
  return (
    input: SetMyAppTagArg<MethodVersion>,
    init?: ClientRequestInit<IHttpClient, SetMyAppTagResponse<MethodVersion>>,
  ): SetMyAppTagResult<MethodVersion, TMethod> => {
    const args = parseVersionedArgs(VersionContract, apiVersion, input);
    return client[method](
      generateApiPath(apiVersion, args),
      generateRequestParameters(apiVersion, args, init),
    ) as SetMyAppTagResult<MethodVersion, TMethod>;
  };
};
