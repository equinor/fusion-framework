...
import { AppGovernanceDocumentRequestSchemaV1 } from '../v1/schemas/app-governance-document-request-schema-v1';

/** Concrete API versions this operation publishes. */
type AvailableVersions = ApiVersion.v1;

/**
 * API version identifiers `createAppGovernanceDocument` accepts for `POST
 ...
 * {@link CreateAppGovernanceDocumentArg} and {@link CreateAppGovernanceDocumentResponse}.
 */
type CreateAppGovernanceDocumentVersion = FilterAllowedApiVersions<AvailableVersions>;

/**
 * Version contract for `createAppGovernanceDocument`, implementing `POST
 * /apps/{appIdentifier}/governance/documents`.
 *
  ...
  [ApiVersion.v1]: {
    ...
    args: z
      .object({
        ...
        ...AppGovernanceDocumentRequestSchemaV1.shape,
      })
      .describe(
        'Arguments for POST /apps/{appIdentifier}/governance/documents (createAppGovernanceDocument v1.0).',
      ),
...
/**
 * Response body `createAppGovernanceDocument` resolves for the selected API version.
 *
 * Version 1.0 answers `201 Created` without a body, so the type resolves to `void`.
 */
type CreateAppGovernanceDocumentResponse<TVersion extends CreateAppGovernanceDocumentVersion> =
  VersionedResponse<typeof VersionContract, ExtractApiVersion<TVersion>>;

/**
 * Arguments `createAppGovernanceDocument` accepts, resolved from the selected API version.
 *
 * Version 1.0 accepts the path identifier `appIdentifier`, and the request body fields of
 * `AppGovernanceDocumentRequestV1`. Path identifiers and body fields share one flat object; the
 ...
 */
type CreateAppGovernanceDocumentArg<TVersion extends CreateAppGovernanceDocumentVersion> =
  VersionedArgs<typeof VersionContract, ExtractApiVersion<TVersion>>;

/**
 * What `createAppGovernanceDocument` hands back once the request runs, for the selected API version
 * and client method.
 *
 * `'json'` gives `Promise<CreateAppGovernanceDocumentResponse<TVersion>>` and `'json$'` gives
 * `StreamResponse<CreateAppGovernanceDocumentResponse<TVersion>>`, so promise and observable
 * callers share one response type.
 */
type CreateAppGovernanceDocumentResult<
  TVersion extends CreateAppGovernanceDocumentVersion,
  TMethod extends ClientMethodType = 'json',
> = ClientMethod<CreateAppGovernanceDocumentResponse<TVersion>>[TMethod];
  ...
  switch (version) {
    case ApiVersion.v1: {
      ...
      const baseInit: FetchRequestInit<
        CreateAppGovernanceDocumentResponse<ApiVersion.v1>,
        JsonRequest
      > = {
        method: 'POST',
...
/**
 * Creates a new governance document for the specified app.
 *
 * Fusion Apps API operation: `POST /apps/{appIdentifier}/governance/documents` — "Creates a new
 * governance document for the specified app."
 *
 * Curried in two stages: `createAppGovernanceDocument(version, client, method)` binds the API
 ...
 * {@link CreateAppGovernanceDocumentArg} plus an optional `ClientRequestInit`, and gives back
 * {@link CreateAppGovernanceDocumentResult}.
 *
 ...
 *
 * Related: `listAppGovernanceDocuments`.
 *
 ...
 * @returns A request function taking {@link CreateAppGovernanceDocumentArg} and an optional
 * `ClientRequestInit`, returning {@link CreateAppGovernanceDocumentResult}.
 ...
 * @example
 * ```ts
 * import { createAppGovernanceDocument } from '@equinor/fusion-services/apps';
 *
 * await createAppGovernanceDocument('v1', httpClient)({
 *   appIdentifier: 'my-app',
 *   content: '# Ownership',
 * });
 * ```
 */
const createAppGovernanceDocument = <
  TVersion extends CreateAppGovernanceDocumentVersion,
  TMethod extends ClientMethodType = 'json',
>(
  version: TVersion,
  ...
  return (
    input: CreateAppGovernanceDocumentArg<MethodVersion>,
    init?: ClientRequestInit<IHttpClient, CreateAppGovernanceDocumentResponse<MethodVersion>>,
  ): CreateAppGovernanceDocumentResult<MethodVersion, TMethod> => {
    const args = parseVersionedArgs(VersionContract, apiVersion, input);
    return client[method](
      generateApiPath(apiVersion, args),
      generateRequestParameters(apiVersion, args, init),
    ) as CreateAppGovernanceDocumentResult<MethodVersion, TMethod>;
  };
};
