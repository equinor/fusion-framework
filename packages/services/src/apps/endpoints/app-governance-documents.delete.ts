...
/**
 * API version identifiers `deleteAppGovernanceDocument` accepts for `DELETE
 ...
 * {@link DeleteAppGovernanceDocumentArg} and {@link DeleteAppGovernanceDocumentResponse}.
 */
type DeleteAppGovernanceDocumentVersion = FilterAllowedApiVersions<AvailableVersions>;

/**
 * Version contract for `deleteAppGovernanceDocument`, implementing `DELETE
 * /apps/{appIdentifier}/governance/documents/{documentType}`.
 *
...
const VersionContract = {
  [ApiVersion.v1]: {
    ...
    args: z
      ...
      .describe(
        'Arguments for DELETE /apps/{appIdentifier}/governance/documents/{documentType} (deleteAppGovernanceDocument v1.0).',
      ),
...
/**
 * Response body `deleteAppGovernanceDocument` resolves for the selected API version.
 *
 * Version 1.0 answers `204 No Content` without a body, so the type resolves to `void`.
 */
type DeleteAppGovernanceDocumentResponse<TVersion extends DeleteAppGovernanceDocumentVersion> =
  VersionedResponse<typeof VersionContract, ExtractApiVersion<TVersion>>;

/**
 * Arguments `deleteAppGovernanceDocument` accepts, resolved from the selected API version.
 *
 ...
 */
type DeleteAppGovernanceDocumentArg<TVersion extends DeleteAppGovernanceDocumentVersion> =
  VersionedArgs<typeof VersionContract, ExtractApiVersion<TVersion>>;

/**
 * What `deleteAppGovernanceDocument` hands back once the request runs, for the selected API version
 * and client method.
 *
 * `'json'` gives `Promise<DeleteAppGovernanceDocumentResponse<TVersion>>` and `'json$'` gives
 * `StreamResponse<DeleteAppGovernanceDocumentResponse<TVersion>>`, so promise and observable
 * callers share one response type.
 */
type DeleteAppGovernanceDocumentResult<
  TVersion extends DeleteAppGovernanceDocumentVersion,
  TMethod extends ClientMethodType = 'json',
> = ClientMethod<DeleteAppGovernanceDocumentResponse<TVersion>>[TMethod];
  ...
  switch (version) {
    case ApiVersion.v1: {
      const baseInit: FetchRequestInit<
        DeleteAppGovernanceDocumentResponse<ApiVersion.v1>,
        JsonRequest
      > = {
        method: 'DELETE',
...
/**
 * Deletes a specific governance document for the specified app.
 *
 * Fusion Apps API operation: `DELETE /apps/{appIdentifier}/governance/documents/{documentType}` —
 * "Deletes a specific governance document for the specified app."
 *
 * Curried in two stages: `deleteAppGovernanceDocument(version, client, method)` binds the API
 ...
 * {@link DeleteAppGovernanceDocumentArg} plus an optional `ClientRequestInit`, and gives back
 * {@link DeleteAppGovernanceDocumentResult}.
 *
 ...
 * @returns A request function taking {@link DeleteAppGovernanceDocumentArg} and an optional
 * `ClientRequestInit`, returning {@link DeleteAppGovernanceDocumentResult}.
 ...
 * @example
 * ```ts
 * import { deleteAppGovernanceDocument } from '@equinor/fusion-services/apps';
 *
 * await deleteAppGovernanceDocument('v1', httpClient)({
 *   appIdentifier: 'my-app',
 *   documentType: 'AppOwnership',
 * });
 * ```
 */
const deleteAppGovernanceDocument = <
  TVersion extends DeleteAppGovernanceDocumentVersion,
  TMethod extends ClientMethodType = 'json',
>(
  version: TVersion,
  ...
  return (
    input: DeleteAppGovernanceDocumentArg<MethodVersion>,
    init?: ClientRequestInit<IHttpClient, DeleteAppGovernanceDocumentResponse<MethodVersion>>,
  ): DeleteAppGovernanceDocumentResult<MethodVersion, TMethod> => {
    const args = parseVersionedArgs(VersionContract, apiVersion, input);
    return client[method](
      generateApiPath(apiVersion, args),
      generateRequestParameters(apiVersion, args, init),
    ) as DeleteAppGovernanceDocumentResult<MethodVersion, TMethod>;
  };
};
