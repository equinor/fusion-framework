...
/**
 * API version identifiers `deleteSystemAccessRoleAssignment` accepts for
 ...
 * {@link DeleteSystemAccessRoleAssignmentArg} and
 * {@link DeleteSystemAccessRoleAssignmentResponse}.
 */
type DeleteSystemAccessRoleAssignmentVersion = FilterAllowedApiVersions<AvailableVersions>;
...
const VersionContract = {
  [ApiVersion.v1]: {
    ...
    args: z
      ...
      .describe(
        'Arguments for DELETE /systems/{systemIdentifier}/access-roles/{accessRoleIdentifier}/assignments/{accessRoleAssignmentId} (deleteSystemAccessRoleAssignment v1.0).',
      ),
...
/**
 * Response body `deleteSystemAccessRoleAssignment` resolves for the selected API version.
 *
...
/**
 * Arguments `deleteSystemAccessRoleAssignment` accepts, resolved from the selected API version.
 *
 ...
 */
type DeleteSystemAccessRoleAssignmentArg<TVersion extends DeleteSystemAccessRoleAssignmentVersion> =
  VersionedArgs<typeof VersionContract, ExtractApiVersion<TVersion>>;

/**
 * What `deleteSystemAccessRoleAssignment` hands back once the request runs, for the selected
 * API version and client method.
 *
 * `'json'` gives `Promise<DeleteSystemAccessRoleAssignmentResponse<TVersion>>` and `'json$'`
 * gives `StreamResponse<DeleteSystemAccessRoleAssignmentResponse<TVersion>>`, so promise and
  ...
  switch (version) {
    case ApiVersion.v1: {
      const baseInit: FetchRequestInit<
        DeleteSystemAccessRoleAssignmentResponse<ApiVersion.v1>,
        JsonRequest
      > = {
        method: 'DELETE',
 ...
 * ```
 */
const deleteSystemAccessRoleAssignment = <
  TVersion extends DeleteSystemAccessRoleAssignmentVersion,
  TMethod extends ClientMethodType = 'json',
>(
  version: TVersion,
