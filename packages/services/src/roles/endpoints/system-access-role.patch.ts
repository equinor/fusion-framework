...
/**
 * API version identifiers `updateSystemAccessRole` accepts for
 ...
 * {@link UpdateSystemAccessRoleArg} and {@link UpdateSystemAccessRoleResponse}.
 */
type UpdateSystemAccessRoleVersion = FilterAllowedApiVersions<AvailableVersions>;
...
const VersionContract = {
  [ApiVersion.v1]: {
    ...
    args: z
      ...
      .describe(
        'Arguments for PATCH /systems/{systemIdentifier}/access-roles/{accessRoleIdentifier} (updateSystemAccessRole v1.0).',
      ),
...
/**
 * Response body `updateSystemAccessRole` resolves for the selected API version.
 *
 ...
 */
type UpdateSystemAccessRoleResponse<TVersion extends UpdateSystemAccessRoleVersion> =
  VersionedResponse<typeof VersionContract, ExtractApiVersion<TVersion>>;

/**
 * Arguments `updateSystemAccessRole` accepts, resolved from the selected API version.
 *
 * Version 1.0 accepts the path identifiers `systemIdentifier`, `accessRoleIdentifier`, and the
 * request body fields of `UpdateAccessRoleRequestV1`.
 *
 ...
 * is sent.
 */
type UpdateSystemAccessRoleArg<TVersion extends UpdateSystemAccessRoleVersion> = VersionedArgs<
  typeof VersionContract,
  ExtractApiVersion<TVersion>
>;

/**
 * What `updateSystemAccessRole` hands back once the request runs, for the selected API version
 * and client method.
 *
 * `'json'` gives `Promise<UpdateSystemAccessRoleResponse<TVersion>>` and `'json$'` gives
 * `StreamResponse<UpdateSystemAccessRoleResponse<TVersion>>`, so promise and observable callers
 * share one response type.
 */
type UpdateSystemAccessRoleResult<
  TVersion extends UpdateSystemAccessRoleVersion,
  TMethod extends ClientMethodType = 'json',
> = ClientMethod<UpdateSystemAccessRoleResponse<TVersion>>[TMethod];
  ...
  switch (version) {
    case ApiVersion.v1: {
      ...
      const baseInit: FetchRequestInit<
        UpdateSystemAccessRoleResponse<ApiVersion.v1>,
        JsonRequest
      > = {
        method: 'PATCH',
...
>(
  ...
  return (
    input: UpdateSystemAccessRoleArg<MethodVersion>,
    init?: ClientRequestInit<IHttpClient, UpdateSystemAccessRoleResponse<MethodVersion>>,
  ): UpdateSystemAccessRoleResult<MethodVersion, TMethod> => {
    const args = parseVersionedArgs(VersionContract, apiVersion, input);
    return client[method](
      generateApiPath(apiVersion, args),
      generateRequestParameters(apiVersion, args, init),
    ) as UpdateSystemAccessRoleResult<MethodVersion, TMethod>;
  };
};
