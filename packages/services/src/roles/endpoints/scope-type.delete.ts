...
/**
 * API version identifiers `deleteScopeType` accepts for
 * `DELETE /scope-types/{scopeTypeIdentifier}`.
 *
 * Roles V2 publishes version 1.0 of this operation, nameable as `'v1'`, `'1.0'`, or
 * `ApiVersion.v1`. All three resolve to the same contract entry, so all three infer the same
 * {@link DeleteScopeTypeArg} and {@link DeleteScopeTypeResponse}.
 */
type DeleteScopeTypeVersion = FilterAllowedApiVersions<AvailableVersions>;

/**
 * Version contract for `deleteScopeType`, implementing
 * `DELETE /scope-types/{scopeTypeIdentifier}`.
 *
...
const VersionContract = {
  [ApiVersion.v1]: {
    ...
    args: z
      ...
      .describe('Arguments for DELETE /scope-types/{scopeTypeIdentifier} (deleteScopeType v1.0).'),
...
/**
 * Response body `deleteScopeType` resolves for the selected API version.
 *
 * Version 1.0 answers `204 No Content` without a body, so the type resolves to `void`.
 */
type DeleteScopeTypeResponse<TVersion extends DeleteScopeTypeVersion> = VersionedResponse<
  typeof VersionContract,
  ExtractApiVersion<TVersion>
>;

/**
 * Arguments `deleteScopeType` accepts, resolved from the selected API version.
 *
 ...
 */
type DeleteScopeTypeArg<TVersion extends DeleteScopeTypeVersion> = VersionedArgs<
  typeof VersionContract,
  ExtractApiVersion<TVersion>
>;

/**
 * What `deleteScopeType` hands back once the request runs, for the selected API version and
 * client method.
 *
 * `'json'` gives `Promise<DeleteScopeTypeResponse<TVersion>>` and `'json$'` gives
 * `StreamResponse<DeleteScopeTypeResponse<TVersion>>`, so promise and observable callers share
 * one response type.
 */
...
const generateRequestParameters = <TResult, TVersion extends AvailableVersions>(
  ...
  switch (version) {
    case ApiVersion.v1: {
      const baseInit: FetchRequestInit<DeleteScopeTypeResponse<ApiVersion.v1>, JsonRequest> = {
        method: 'DELETE',
...
/**
 * Deletes a scope type, addressed by its UUID or unique name.
 *
 * Roles V2 operation: `DELETE /scope-types/{scopeTypeIdentifier}` — "Delete a scope type by
 * identifier."
 *
 * Curried in two stages: `deleteScopeType(version, client, method)` binds the API version, the
 ...
 * {@link DeleteScopeTypeArg} plus an optional `ClientRequestInit`, and gives back
 * {@link DeleteScopeTypeResult}.
 *
 ...
 *
 * Destructive: it removes the scope type permanently, and the deletion cannot be undone through
 * this API. Roles V2 answers `204 No Content` with no body, so the resolved value is `void`.
 ...
 * @returns A request function taking {@link DeleteScopeTypeArg} and an optional
 * `ClientRequestInit`, returning {@link DeleteScopeTypeResult}.
 ...
 * @example
 * ```ts
 * import { deleteScopeType } from '@equinor/fusion-services/roles';
 *
 * await deleteScopeType('v1', httpClient)({ scopeTypeIdentifier: 'project' });
 * ```
 */
const deleteScopeType = <
  TVersion extends DeleteScopeTypeVersion,
  TMethod extends ClientMethodType = 'json',
>(
  version: TVersion,
