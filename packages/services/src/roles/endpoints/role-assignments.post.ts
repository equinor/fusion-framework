...
/**
 * API version identifiers `assignRole` accepts for `POST /roles/{roleIdentifier}/assignments`.
 *
 * Roles V2 publishes version 1.0 of this operation, nameable as `'v1'`, `'1.0'`, or
 * `ApiVersion.v1`. All three resolve to the same contract entry, so all three infer the same
 * {@link AssignRoleArg} and {@link AssignRoleResponse}.
 */
type AssignRoleVersion = FilterAllowedApiVersions<AvailableVersions>;
...
/**
 * Response body `assignRole` resolves for the selected API version.
 *
 * Version 1.0 resolves to `ApiRoleAssignmentV1`, inferred from `ApiRoleAssignmentSchemaV1` —
 * the very schema that validates the `201 Created` body at runtime.
 */
type AssignRoleResponse<TVersion extends AssignRoleVersion> = VersionedResponse<
  typeof VersionContract,
  ExtractApiVersion<TVersion>
>;

/**
 * Arguments `assignRole` accepts, resolved from the selected API version.
 *
 ...
 * is sent.
 */
type AssignRoleArg<TVersion extends AssignRoleVersion> = VersionedArgs<
  typeof VersionContract,
  ExtractApiVersion<TVersion>
>;

/**
 * What `assignRole` hands back once the request runs, for the selected API version and client
 * method.
 *
 * `'json'` gives `Promise<AssignRoleResponse<TVersion>>` and `'json$'` gives
 * `StreamResponse<AssignRoleResponse<TVersion>>`, so promise and observable callers share one
 * response type.
 */
type AssignRoleResult<
  TVersion extends AssignRoleVersion,
  TMethod extends ClientMethodType = 'json',
> = ClientMethod<AssignRoleResponse<TVersion>>[TMethod];
...
const generateRequestParameters = <TResult, TVersion extends AvailableVersions>(
  ...
  switch (version) {
    case ApiVersion.v1: {
      ...
      const baseInit: FetchRequestInit<AssignRoleResponse<ApiVersion.v1>, JsonRequest> = {
        method: 'POST',
 ...
 * ```
 */
const assignRole = <TVersion extends AssignRoleVersion, TMethod extends ClientMethodType = 'json'>(
  version: TVersion,
  client: IHttpClient,
  ...
  return (
    input: AssignRoleArg<MethodVersion>,
    init?: ClientRequestInit<IHttpClient, AssignRoleResponse<MethodVersion>>,
  ): AssignRoleResult<MethodVersion, TMethod> => {
    const args = parseVersionedArgs(VersionContract, apiVersion, input);
    return client[method](
      generateApiPath(apiVersion, args),
      generateRequestParameters(apiVersion, args, init),
    ) as AssignRoleResult<MethodVersion, TMethod>;
  };
};
