...
/**
 * API version identifiers `getScopeType` accepts for `GET /scope-types/{scopeTypeIdentifier}`.
 *
 * Roles V2 publishes version 1.0 of this operation, nameable as `'v1'`, `'1.0'`, or
 * `ApiVersion.v1`. All three resolve to the same contract entry, so all three infer the same
 * {@link GetScopeTypeArg} and {@link GetScopeTypeResponse}.
 */
type GetScopeTypeVersion = FilterAllowedApiVersions<AvailableVersions>;
...
/**
 * Response body `getScopeType` resolves for the selected API version.
 *
 ...
 */
type GetScopeTypeResponse<TVersion extends GetScopeTypeVersion> = VersionedResponse<
  typeof VersionContract,
  ExtractApiVersion<TVersion>
>;

/**
 * Arguments `getScopeType` accepts, resolved from the selected API version.
 *
 * Version 1.0 accepts the path identifier `scopeTypeIdentifier`.
 *
 ...
 */
type GetScopeTypeArg<TVersion extends GetScopeTypeVersion> = VersionedArgs<
  typeof VersionContract,
  ExtractApiVersion<TVersion>
>;

/**
 * What `getScopeType` hands back once the request runs, for the selected API version and client
 * method.
 *
 * `'json'` gives `Promise<GetScopeTypeResponse<TVersion>>` and `'json$'` gives
 * `StreamResponse<GetScopeTypeResponse<TVersion>>`, so promise and observable callers share one
 * response type.
 */
...
const generateRequestParameters = <TResult, TVersion extends AvailableVersions>(
  ...
  switch (version) {
    case ApiVersion.v1: {
      const baseInit: FetchRequestInit<GetScopeTypeResponse<ApiVersion.v1>, JsonRequest> = {
