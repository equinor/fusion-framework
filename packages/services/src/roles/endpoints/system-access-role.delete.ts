...
/**
 * API version identifiers `deleteSystemAccessRole` accepts for
 ...
 * {@link DeleteSystemAccessRoleArg} and {@link DeleteSystemAccessRoleResponse}.
 */
type DeleteSystemAccessRoleVersion = FilterAllowedApiVersions<AvailableVersions>;

/**
 * Version contract for `deleteSystemAccessRole`, implementing
 * `DELETE /systems/{systemIdentifier}/access-roles/{accessRoleIdentifier}`.
 *
...
const VersionContract = {
  [ApiVersion.v1]: {
    ...
    args: z
      ...
      .describe(
        'Arguments for DELETE /systems/{systemIdentifier}/access-roles/{accessRoleIdentifier} (deleteSystemAccessRole v1.0).',
      ),
...
/**
 * Response body `deleteSystemAccessRole` resolves for the selected API version.
 *
 * Version 1.0 answers `204 No Content` without a body, so the type resolves to `void`.
 */
type DeleteSystemAccessRoleResponse<TVersion extends DeleteSystemAccessRoleVersion> =
  VersionedResponse<typeof VersionContract, ExtractApiVersion<TVersion>>;

/**
 * Arguments `deleteSystemAccessRole` accepts, resolved from the selected API version.
 *
 ...
 */
type DeleteSystemAccessRoleArg<TVersion extends DeleteSystemAccessRoleVersion> = VersionedArgs<
  typeof VersionContract,
  ExtractApiVersion<TVersion>
>;

/**
 * What `deleteSystemAccessRole` hands back once the request runs, for the selected API version
 * and client method.
 *
 * `'json'` gives `Promise<DeleteSystemAccessRoleResponse<TVersion>>` and `'json$'` gives
 * `StreamResponse<DeleteSystemAccessRoleResponse<TVersion>>`, so promise and observable callers
 * share one response type.
 */
type DeleteSystemAccessRoleResult<
  TVersion extends DeleteSystemAccessRoleVersion,
  TMethod extends ClientMethodType = 'json',
> = ClientMethod<DeleteSystemAccessRoleResponse<TVersion>>[TMethod];
  ...
  switch (version) {
    case ApiVersion.v1: {
      const baseInit: FetchRequestInit<
        DeleteSystemAccessRoleResponse<ApiVersion.v1>,
        JsonRequest
      > = {
        method: 'DELETE',
...
/**
 * Deletes one access role owned by a system.
 *
 * Roles V2 operation: `DELETE /systems/{systemIdentifier}/access-roles/{accessRoleIdentifier}`
 * — "Delete an access role from a system."
 *
 * Curried in two stages: `deleteSystemAccessRole(version, client, method)` binds the API
 ...
 * {@link DeleteSystemAccessRoleArg} plus an optional `ClientRequestInit`, and gives back
 * {@link DeleteSystemAccessRoleResult}.
 *
 ...
 *
 * Destructive: it removes the access role permanently, and the deletion cannot be undone
 ...
 * @returns A request function taking {@link DeleteSystemAccessRoleArg} and an optional
 * `ClientRequestInit`, returning {@link DeleteSystemAccessRoleResult}.
 ...
 * @example
 * ```ts
 * import { deleteSystemAccessRole } from '@equinor/fusion-services/roles';
 *
 * await deleteSystemAccessRole('v1', httpClient)({
 *   systemIdentifier: 'fusion-core',
 *   accessRoleIdentifier: 'writer',
 * });
 * ```
 */
const deleteSystemAccessRole = <
  TVersion extends DeleteSystemAccessRoleVersion,
  TMethod extends ClientMethodType = 'json',
>(
  version: TVersion,
  ...
  return (
    input: DeleteSystemAccessRoleArg<MethodVersion>,
    init?: ClientRequestInit<IHttpClient, DeleteSystemAccessRoleResponse<MethodVersion>>,
  ): DeleteSystemAccessRoleResult<MethodVersion, TMethod> => {
    const args = parseVersionedArgs(VersionContract, apiVersion, input);
    return client[method](
      generateApiPath(apiVersion, args),
      generateRequestParameters(apiVersion, args, init),
    ) as DeleteSystemAccessRoleResult<MethodVersion, TMethod>;
  };
};
