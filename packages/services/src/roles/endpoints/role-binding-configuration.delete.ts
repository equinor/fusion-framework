...
/**
 * API version identifiers `deleteRoleBindingConfiguration` accepts for
 * `DELETE /role-binding-configurations/{identifier}`.
 *
 * Roles V2 publishes version 1.0 of this operation, nameable as `'v1'`, `'1.0'`, or
 * `ApiVersion.v1`. All three resolve to the same contract entry, so all three infer the same
 * {@link DeleteRoleBindingConfigurationArg} and {@link DeleteRoleBindingConfigurationResponse}.
 */
type DeleteRoleBindingConfigurationVersion = FilterAllowedApiVersions<AvailableVersions>;

/**
 * Version contract for `deleteRoleBindingConfiguration`, implementing
 * `DELETE /role-binding-configurations/{identifier}`.
 *
...
const VersionContract = {
  [ApiVersion.v1]: {
    ...
    args: z
      ...
      .describe(
        'Arguments for DELETE /role-binding-configurations/{identifier} (deleteRoleBindingConfiguration v1.0).',
      ),
...
/**
 * Response body `deleteRoleBindingConfiguration` resolves for the selected API version.
 *
...
/**
 * Arguments `deleteRoleBindingConfiguration` accepts, resolved from the selected API version.
 *
 ...
 */
type DeleteRoleBindingConfigurationArg<TVersion extends DeleteRoleBindingConfigurationVersion> =
  VersionedArgs<typeof VersionContract, ExtractApiVersion<TVersion>>;

/**
 * What `deleteRoleBindingConfiguration` hands back once the request runs, for the selected API
 * version and client method.
 *
 * `'json'` gives `Promise<DeleteRoleBindingConfigurationResponse<TVersion>>` and `'json$'`
 * gives `StreamResponse<DeleteRoleBindingConfigurationResponse<TVersion>>`, so promise and
  ...
  switch (version) {
    case ApiVersion.v1: {
      const baseInit: FetchRequestInit<
        DeleteRoleBindingConfigurationResponse<ApiVersion.v1>,
        JsonRequest
      > = {
        method: 'DELETE',
...
/**
 * Deletes a role binding configuration by its identifier.
 *
 * Roles V2 operation: `DELETE /role-binding-configurations/{identifier}` — "Delete a role
 * binding configuration by identifier."
 *
 * Curried in two stages: `deleteRoleBindingConfiguration(version, client, method)` binds the
 ...
 * {@link DeleteRoleBindingConfigurationArg} plus an optional `ClientRequestInit`, and gives
 * back {@link DeleteRoleBindingConfigurationResult}.
 *
 ...
 *
 * Destructive: it removes the role binding configuration permanently, and the deletion cannot
 * be undone through this API. Roles V2 answers `204 No Content` with no body, so the resolved
 ...
 * @returns A request function taking {@link DeleteRoleBindingConfigurationArg} and an optional
 * `ClientRequestInit`, returning {@link DeleteRoleBindingConfigurationResult}.
 ...
 * @example
 * ```ts
 * import { deleteRoleBindingConfiguration } from '@equinor/fusion-services/roles';
 *
 * await deleteRoleBindingConfiguration('v1', httpClient)({ identifier: 'org-chart-sync' });
 * ```
 */
const deleteRoleBindingConfiguration = <
  TVersion extends DeleteRoleBindingConfigurationVersion,
  TMethod extends ClientMethodType = 'json',
>(
  version: TVersion,
