...
import { UpdateRoleBindingConfigurationRequestSchemaV1 } from '../v1/schemas/update-role-binding-configuration-request-schema-v1';

/** Concrete API versions this operation publishes. */
type AvailableVersions = ApiVersion.v1;

/**
 * API version identifiers `updateRoleBindingConfiguration` accepts for
 * `PATCH /role-binding-configurations/{identifier}`.
 *
 * Roles V2 publishes version 1.0 of this operation, nameable as `'v1'`, `'1.0'`, or
 * `ApiVersion.v1`. All three resolve to the same contract entry, so all three infer the same
 * {@link UpdateRoleBindingConfigurationArg} and {@link UpdateRoleBindingConfigurationResponse}.
 */
type UpdateRoleBindingConfigurationVersion = FilterAllowedApiVersions<AvailableVersions>;

/**
 * Version contract for `updateRoleBindingConfiguration`, implementing
 * `PATCH /role-binding-configurations/{identifier}`.
 *
  ...
  [ApiVersion.v1]: {
    ...
    args: z
      .object({
        ...
        ...UpdateRoleBindingConfigurationRequestSchemaV1.shape,
      })
      .describe(
        'Arguments for PATCH /role-binding-configurations/{identifier} (updateRoleBindingConfiguration v1.0).',
      ),
...
/**
 * Response body `updateRoleBindingConfiguration` resolves for the selected API version.
 *
...
/**
 * Arguments `updateRoleBindingConfiguration` accepts, resolved from the selected API version.
 *
 * Version 1.0 accepts the path identifier `identifier`, and the request body fields of
 * `UpdateRoleBindingConfigurationRequestV1`.
 *
 ...
 * is sent.
 */
type UpdateRoleBindingConfigurationArg<TVersion extends UpdateRoleBindingConfigurationVersion> =
  VersionedArgs<typeof VersionContract, ExtractApiVersion<TVersion>>;

/**
 * What `updateRoleBindingConfiguration` hands back once the request runs, for the selected API
 * version and client method.
 *
 * `'json'` gives `Promise<UpdateRoleBindingConfigurationResponse<TVersion>>` and `'json$'`
 * gives `StreamResponse<UpdateRoleBindingConfigurationResponse<TVersion>>`, so promise and
  ...
  switch (version) {
    case ApiVersion.v1: {
      ...
      const baseInit: FetchRequestInit<
        UpdateRoleBindingConfigurationResponse<ApiVersion.v1>,
        JsonRequest
      > = {
        method: 'PATCH',
...
const generateApiPath = <TVersion extends AvailableVersions>(
  ...
  switch (version) {
    case ApiVersion.v1: {
      ...
      return `/role-binding-configurations/${encodeURIComponent(args.identifier)}?${String(params)}`;
    }
  }
  throw Error(`Unknown API version: ${version}`);
};
...
/**
 * Updates the binding rules and metadata of a role binding configuration.
 *
 * Roles V2 operation: `PATCH /role-binding-configurations/{identifier}` — "Update a role
 * binding configuration by identifier."
 *
 * Curried in two stages: `updateRoleBindingConfiguration(version, client, method)` binds the
 ...
 * {@link UpdateRoleBindingConfigurationArg} plus an optional `ClientRequestInit`, and gives
 * back {@link UpdateRoleBindingConfigurationResult}.
 *
 ...
 *
 * Roles V2 answers `204 No Content` with no body, so the resolved value is `void`. Fields left
 * out of the arguments are not sent, so they keep their stored values. The contract declares
 ...
 * @returns A request function taking {@link UpdateRoleBindingConfigurationArg} and an optional
 * `ClientRequestInit`, returning {@link UpdateRoleBindingConfigurationResult}.
 ...
 * @example
 * ```ts
 * import { updateRoleBindingConfiguration } from '@equinor/fusion-services/roles';
 *
 * await updateRoleBindingConfiguration('v1', httpClient)({
 *   identifier: 'org-chart-sync',
 *   description: 'Sync approvers from the org chart',
 * });
 * ```
 */
const updateRoleBindingConfiguration = <
  TVersion extends UpdateRoleBindingConfigurationVersion,
  TMethod extends ClientMethodType = 'json',
>(
  version: TVersion,
