...
/**
 * API version identifiers `createSystemAccessRole` accepts for
 ...
 * {@link CreateSystemAccessRoleArg} and {@link CreateSystemAccessRoleResponse}.
 */
type CreateSystemAccessRoleVersion = FilterAllowedApiVersions<AvailableVersions>;
...
const VersionContract = {
  [ApiVersion.v1]: {
    ...
    args: z
      ...
      .describe(
        'Arguments for POST /systems/{systemIdentifier}/access-roles (createSystemAccessRole v1.0).',
      ),
...
/**
 * Response body `createSystemAccessRole` resolves for the selected API version.
 *
 ...
 */
type CreateSystemAccessRoleResponse<TVersion extends CreateSystemAccessRoleVersion> =
  VersionedResponse<typeof VersionContract, ExtractApiVersion<TVersion>>;

/**
 * Arguments `createSystemAccessRole` accepts, resolved from the selected API version.
 *
 * Version 1.0 accepts the path identifier `systemIdentifier`, and the request body fields of
 * `CreateAccessRoleRequestV1`.
 *
 ...
 * is sent.
 */
type CreateSystemAccessRoleArg<TVersion extends CreateSystemAccessRoleVersion> = VersionedArgs<
  typeof VersionContract,
  ExtractApiVersion<TVersion>
>;

/**
 * What `createSystemAccessRole` hands back once the request runs, for the selected API version
 * and client method.
 *
 * `'json'` gives `Promise<CreateSystemAccessRoleResponse<TVersion>>` and `'json$'` gives
 * `StreamResponse<CreateSystemAccessRoleResponse<TVersion>>`, so promise and observable callers
  ...
  switch (version) {
    case ApiVersion.v1: {
      ...
      const baseInit: FetchRequestInit<
        CreateSystemAccessRoleResponse<ApiVersion.v1>,
        JsonRequest
      > = {
        method: 'POST',
