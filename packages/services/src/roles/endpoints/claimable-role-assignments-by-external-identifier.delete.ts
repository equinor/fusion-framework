...
const VersionContract = {
  [ApiVersion.v1]: {
    ...
    args: z
      ...
      .describe(
        'Arguments for DELETE /claimable-role-assignments/{externalIdentifier} (deleteClaimableRoleAssignmentsByExternalIdentifier v1.0).',
      ),
  ...
  switch (version) {
    case ApiVersion.v1: {
      const baseInit: FetchRequestInit<
        DeleteClaimableRoleAssignmentsByExternalIdentifierResponse<ApiVersion.v1>,
        JsonRequest
      > = {
        method: 'DELETE',
...
const generateApiPath = <TVersion extends AvailableVersions>(
  ...
  switch (version) {
    case ApiVersion.v1: {
      ...
      return `/claimable-role-assignments/${encodeURIComponent(args.externalIdentifier)}?${String(params)}`;
    }
  }
  throw Error(`Unknown API version: ${version}`);
};
 ...
 * ```
 */
const deleteClaimableRoleAssignmentsByExternalIdentifier = <
  TVersion extends DeleteClaimableRoleAssignmentsByExternalIdentifierVersion,
  TMethod extends ClientMethodType = 'json',
>(
  version: TVersion,
