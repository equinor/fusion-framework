...
const VersionContract = {
  [ApiVersion.v1]: {
    ...
    args: z
      ...
      .describe(
        'Arguments for GET /claimable-roles/{claimableRoleIdentifier}/assignments/{claimableRoleAssignmentId}/activations (listActivationsForClaimableRoleAssignment v1.0).',
      ),
    /** Response published by version 1.0 of this operation. */
    response: apiPagedCollectionSchemaV1(ApiClaimableRoleAssignmentActivationSchemaV1),
  },
...
type ListActivationsForClaimableRoleAssignmentResponse<
  TVersion extends ListActivationsForClaimableRoleAssignmentVersion,
...
> = ClientMethod<ListActivationsForClaimableRoleAssignmentResponse<TVersion>>[TMethod];
  ...
  switch (version) {
    case ApiVersion.v1: {
      const baseInit: FetchRequestInit<
        ListActivationsForClaimableRoleAssignmentResponse<ApiVersion.v1>,
        JsonRequest
      > = {
...
const generateApiPath = <TVersion extends AvailableVersions>(
  ...
  switch (version) {
    case ApiVersion.v1: {
      ...
      return `/claimable-roles/${encodeURIComponent(args.claimableRoleIdentifier)}/assignments/${encodeURIComponent(args.claimableRoleAssignmentId)}/activations?${String(params)}`;
    }
  }
  throw Error(`Unknown API version: ${version}`);
};
 ...
 * ```
 */
const listActivationsForClaimableRoleAssignment = <
  TVersion extends ListActivationsForClaimableRoleAssignmentVersion,
  TMethod extends ClientMethodType = 'json',
>(
  version: TVersion,
  ...
  return (
    input: ListActivationsForClaimableRoleAssignmentArg<MethodVersion>,
    init?: ClientRequestInit<
      IHttpClient,
      ListActivationsForClaimableRoleAssignmentResponse<MethodVersion>
    >,
  ): ListActivationsForClaimableRoleAssignmentResult<MethodVersion, TMethod> => {
    const args = parseVersionedArgs(VersionContract, apiVersion, input);
    return client[method](
      generateApiPath(apiVersion, args),
      generateRequestParameters(apiVersion, args, init),
    ) as ListActivationsForClaimableRoleAssignmentResult<MethodVersion, TMethod>;
  };
};
