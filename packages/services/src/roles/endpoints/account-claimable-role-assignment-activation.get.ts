  ...
  [ApiVersion.v1]: {
    ...
    args: z
      .object({
        ...
        /** Claimable role assignment activation UUID. */
        claimableRoleAssignmentActivationId: z
          .string()
          .describe('Claimable role assignment activation UUID.'),
      })
      .describe(
        'Arguments for GET /accounts/{accountIdentifier}/claimable-role-assignments/{claimableRoleAssignmentId}/activations/{claimableRoleAssignmentActivationId} (getClaimableRoleAssignmentActivation v1.0).',
      ),
...
type GetClaimableRoleAssignmentActivationResponse<
  TVersion extends GetClaimableRoleAssignmentActivationVersion,
...
> = ClientMethod<GetClaimableRoleAssignmentActivationResponse<TVersion>>[TMethod];
  ...
  switch (version) {
    case ApiVersion.v1: {
      const baseInit: FetchRequestInit<
        GetClaimableRoleAssignmentActivationResponse<ApiVersion.v1>,
        JsonRequest
      > = {
...
const generateApiPath = <TVersion extends AvailableVersions>(
  ...
  switch (version) {
    case ApiVersion.v1: {
      ...
      return `/accounts/${encodeURIComponent(args.accountIdentifier)}/claimable-role-assignments/${encodeURIComponent(args.claimableRoleAssignmentId)}/activations/${encodeURIComponent(args.claimableRoleAssignmentActivationId)}?${String(params)}`;
    }
  }
  throw Error(`Unknown API version: ${version}`);
};
 ...
 * ```
 */
const getClaimableRoleAssignmentActivation = <
  TVersion extends GetClaimableRoleAssignmentActivationVersion,
  TMethod extends ClientMethodType = 'json',
>(
  version: TVersion,
  ...
  return (
    input: GetClaimableRoleAssignmentActivationArg<MethodVersion>,
    init?: ClientRequestInit<
      IHttpClient,
      GetClaimableRoleAssignmentActivationResponse<MethodVersion>
    >,
  ): GetClaimableRoleAssignmentActivationResult<MethodVersion, TMethod> => {
    const args = parseVersionedArgs(VersionContract, apiVersion, input);
    return client[method](
      generateApiPath(apiVersion, args),
      generateRequestParameters(apiVersion, args, init),
    ) as GetClaimableRoleAssignmentActivationResult<MethodVersion, TMethod>;
  };
};
