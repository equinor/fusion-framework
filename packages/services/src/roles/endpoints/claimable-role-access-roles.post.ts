  ...
  [ApiVersion.v1]: {
    ...
    args: z
      .object({
        ...
        ...AddClaimableRoleAccessRoleRequestSchemaV1.shape,
      })
      .describe(
        'Arguments for POST /claimable-roles/{claimableRoleIdentifier}/access-roles (addClaimableRoleAccessRoles v1.0).',
      ),
 ...
 */
type AddClaimableRoleAccessRolesResponse<TVersion extends AddClaimableRoleAccessRolesVersion> =
 ...
 * is sent.
 */
type AddClaimableRoleAccessRolesArg<TVersion extends AddClaimableRoleAccessRolesVersion> =
  ...
  switch (version) {
    case ApiVersion.v1: {
      ...
      const baseInit: FetchRequestInit<
        AddClaimableRoleAccessRolesResponse<ApiVersion.v1>,
        JsonRequest
      > = {
        method: 'POST',
...
const generateApiPath = <TVersion extends AvailableVersions>(
  ...
  switch (version) {
    case ApiVersion.v1: {
      ...
      return `/claimable-roles/${encodeURIComponent(args.claimableRoleIdentifier)}/access-roles?${String(params)}`;
    }
  }
  throw Error(`Unknown API version: ${version}`);
};
 ...
 * ```
 */
const addClaimableRoleAccessRoles = <
  TVersion extends AddClaimableRoleAccessRolesVersion,
  TMethod extends ClientMethodType = 'json',
>(
  version: TVersion,
