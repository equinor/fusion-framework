...
/**
 * API version identifiers `deleteClaimableRole` accepts for
 * `DELETE /claimable-roles/{claimableRoleIdentifier}`.
 *
 * Roles V2 publishes version 1.0 of this operation, nameable as `'v1'`, `'1.0'`, or
 * `ApiVersion.v1`. All three resolve to the same contract entry, so all three infer the same
 * {@link DeleteClaimableRoleArg} and {@link DeleteClaimableRoleResponse}.
 */
type DeleteClaimableRoleVersion = FilterAllowedApiVersions<AvailableVersions>;

/**
 * Version contract for `deleteClaimableRole`, implementing
 * `DELETE /claimable-roles/{claimableRoleIdentifier}`.
 *
...
const VersionContract = {
  [ApiVersion.v1]: {
    ...
    args: z
      ...
      .describe(
        'Arguments for DELETE /claimable-roles/{claimableRoleIdentifier} (deleteClaimableRole v1.0).',
      ),
...
/**
 * Response body `deleteClaimableRole` resolves for the selected API version.
 *
 * Version 1.0 answers `204 No Content` without a body, so the type resolves to `void`.
 */
type DeleteClaimableRoleResponse<TVersion extends DeleteClaimableRoleVersion> = VersionedResponse<
  typeof VersionContract,
  ExtractApiVersion<TVersion>
>;

/**
 * Arguments `deleteClaimableRole` accepts, resolved from the selected API version.
 *
 ...
 */
type DeleteClaimableRoleArg<TVersion extends DeleteClaimableRoleVersion> = VersionedArgs<
  typeof VersionContract,
  ExtractApiVersion<TVersion>
>;

/**
 * What `deleteClaimableRole` hands back once the request runs, for the selected API version and
 * client method.
 *
 * `'json'` gives `Promise<DeleteClaimableRoleResponse<TVersion>>` and `'json$'` gives
 * `StreamResponse<DeleteClaimableRoleResponse<TVersion>>`, so promise and observable callers
...
const generateRequestParameters = <TResult, TVersion extends AvailableVersions>(
  ...
  switch (version) {
    case ApiVersion.v1: {
      const baseInit: FetchRequestInit<DeleteClaimableRoleResponse<ApiVersion.v1>, JsonRequest> = {
        method: 'DELETE',
...
/**
 * Deletes a claimable role, addressed by its UUID or unique name.
 *
 * Roles V2 operation: `DELETE /claimable-roles/{claimableRoleIdentifier}` — "Delete a claimable
 * role by identifier."
 *
 * Curried in two stages: `deleteClaimableRole(version, client, method)` binds the API version,
 ...
 * {@link DeleteClaimableRoleArg} plus an optional `ClientRequestInit`, and gives back
 * {@link DeleteClaimableRoleResult}.
 *
 ...
 *
 * Destructive: it removes the claimable role and its assignments permanently, and the deletion
 * cannot be undone through this API. Roles V2 answers `204 No Content` with no body, so the
 * resolved value is `void`. The contract declares `403 Forbidden` for callers the Roles service
 ...
 * @returns A request function taking {@link DeleteClaimableRoleArg} and an optional
 * `ClientRequestInit`, returning {@link DeleteClaimableRoleResult}.
 ...
 * @example
 * ```ts
 * import { deleteClaimableRole } from '@equinor/fusion-services/roles';
 *
 * await deleteClaimableRole('v1', httpClient)({ claimableRoleIdentifier: 'on-call-approver' });
 * ```
 */
