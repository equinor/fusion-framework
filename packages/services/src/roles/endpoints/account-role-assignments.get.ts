...
> = ClientMethod<ListAccountRoleAssignmentsResponse<TVersion>>[TMethod];
...
>(
  ...
  return (
    ...
    init?: ClientRequestInit<IHttpClient, ListAccountRoleAssignmentsResponse<MethodVersion>>,
  ): ListAccountRoleAssignmentsResult<MethodVersion, TMethod> => {
    const args = parseVersionedArgs(VersionContract, apiVersion, input);
    return client[method](
      generateApiPath(apiVersion, args),
      generateRequestParameters(apiVersion, args, init),
    ) as ListAccountRoleAssignmentsResult<MethodVersion, TMethod>;
  };
};
