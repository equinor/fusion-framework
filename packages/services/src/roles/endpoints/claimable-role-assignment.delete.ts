...
const VersionContract = {
  [ApiVersion.v1]: {
    ...
    args: z
      ...
      .describe(
        'Arguments for DELETE /claimable-roles/{claimableRoleIdentifier}/assignments/{claimableRoleAssignmentId} (deleteClaimableRoleAssignment v1.0).',
      ),
 ...
 */
type DeleteClaimableRoleAssignmentResponse<TVersion extends DeleteClaimableRoleAssignmentVersion> =
 ...
 */
type DeleteClaimableRoleAssignmentArg<TVersion extends DeleteClaimableRoleAssignmentVersion> =
...
type DeleteClaimableRoleAssignmentResult<
  TVersion extends DeleteClaimableRoleAssignmentVersion,
  TMethod extends ClientMethodType = 'json',
> = ClientMethod<DeleteClaimableRoleAssignmentResponse<TVersion>>[TMethod];
  ...
  switch (version) {
    case ApiVersion.v1: {
      const baseInit: FetchRequestInit<
        DeleteClaimableRoleAssignmentResponse<ApiVersion.v1>,
        JsonRequest
      > = {
        method: 'DELETE',
 ...
 * ```
 */
const deleteClaimableRoleAssignment = <
  TVersion extends DeleteClaimableRoleAssignmentVersion,
  TMethod extends ClientMethodType = 'json',
>(
  version: TVersion,
  ...
  return (
    input: DeleteClaimableRoleAssignmentArg<MethodVersion>,
    init?: ClientRequestInit<IHttpClient, DeleteClaimableRoleAssignmentResponse<MethodVersion>>,
  ): DeleteClaimableRoleAssignmentResult<MethodVersion, TMethod> => {
    const args = parseVersionedArgs(VersionContract, apiVersion, input);
    return client[method](
      generateApiPath(apiVersion, args),
      generateRequestParameters(apiVersion, args, init),
    ) as DeleteClaimableRoleAssignmentResult<MethodVersion, TMethod>;
  };
};
