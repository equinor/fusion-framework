...
export const AssignClaimableRoleRequestSchemaV1 = z
  .object({
    /** Mail or Azure unique ID of the account to assign the claimable role to. */
    accountIdentifier: z
      .string()
      .optional()
      .describe('Mail or Azure unique ID of the account to assign the claimable role to.'),
    ...
    scope: RequestClaimableRoleScopeSchemaV1.nullish().describe(
      'Scope restricting the assignment.',
    ),
  ...
  .describe('The body of an assign-claimable-role request.');

/**
 * Request body for assigning a claimable role to an account.
 *
 * Roles API 1.0 model inferred from {@link AssignClaimableRoleRequestSchemaV1}, so
 * `AssignClaimableRoleRequestV1` and the runtime validator can never describe different shapes.
 */
export type AssignClaimableRoleRequestV1 = z.infer<typeof AssignClaimableRoleRequestSchemaV1>;
