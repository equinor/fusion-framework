...
/** Registry entry the `check:openapi roles` invocation resolves to. */
export const ROLES_SERVICE = OPENAPI_SERVICES.find(
  (service) => service.name === 'roles',
) as (typeof OPENAPI_SERVICES)[number];

/** Roles API 1.0 configuration: snapshot path, remote URL, and the version it is pinned to. */
export const ROLES_V1: OpenApiVersionConfig = ROLES_SERVICE.versions[0];

/**
 * The checked-in Roles V2 snapshot, parsed from the same file the drift check reads.
 *
 * Parsing the file directly keeps the suite offline and gives the contract one source of truth:
 * `pnpm check:openapi roles` proves the snapshot matches the live service, and these tests prove
 * the package implements the snapshot.
 */
export const ROLES_OPENAPI_SNAPSHOT: OpenApiDocument = JSON.parse(
  readFileSync(new URL(`../../../../${ROLES_V1.snapshotPath}`, import.meta.url), 'utf8'),
) as OpenApiDocument;
 ...
 * ```
 */
export const listOperations = (document: OpenApiDocument = ROLES_OPENAPI_SNAPSHOT): string[] => {
...
export const ROLES_OPENAPI_OPERATIONS: readonly string[] = listOperations();
