...
import type { ApiRoute, JsonData, ApiProxyHandler } from './types';
 ...
 */
export type ApiDataProcessor<TResponse extends JsonData, TResult extends JsonData> = (
  data: TResponse,
) => {
  data: TResult;
  routes?: ApiRoute[];
...
>(
  ...
  const { route = '/@services', proxyOptions = {} } = args ?? {};
      ...
      const configure = (proxyServer: ProxyServer, options: ProxyOptions) => {
        proxyServer.on('proxyRes', async (proxyReq, req, res) => {
          const interceptor = responseInterceptor(async (responseBuffer): Promise<string> => {
            ...
            const { data, routes } = generateRoutes(response);
            apiRoutes = routes;
