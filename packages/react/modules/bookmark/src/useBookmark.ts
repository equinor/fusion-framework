...
export type RequiredModules = ModulesInstance<[BookmarkModule]>;
...
export const useBookmark = (args?: useBookmarkArgs): useBookmarkResult => {
    const { payloadGenerator, modules } = args ?? ({} as useBookmarkArgs);
    const currentModules = useModules<RequiredModules>();
    const { bookmark: bookmarkProvider } = modules ?? currentModules;
    ...
    const addBookmarkCreator = useCallback(
        (cb: BookmarkPayloadGenerator) => bookmarkProvider.addPayloadGenerator(cb),
        [bookmarkProvider],
    );
