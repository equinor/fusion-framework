...
export const useBookmark = <TData>(bookmarkProvider: IBookmarkProvider): Bookmarks<TData> => {
    ...
    const hasBookmark = useObservableState(bookmarkProvider.hasBookmark$, {
        initial: bookmarkProvider.hasBookmark,
    }).next;

    const bookmarks = useObservableState(bookmarkProvider.bookmarks$, {
        initial: [],
    }).next;
