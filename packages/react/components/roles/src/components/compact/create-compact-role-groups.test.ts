...
describe('createCompactRoleGroups', () => {
  ...
  it('keeps claimed roles available for deactivation and preserves effective access rows', () => {
    const claimed = assignment('claimed', { isActive: true });
    const groups = createCompactRoleGroups(
      [
        { accessRoleName: 'Claimed role', assignmentType: 'Global' },
        {
    ...
    expect(groups.activeAccess).toHaveLength(2);
    expect(groups.activeAccess[1]).toMatchObject({
      displayName: 'Reports.Read',
      description: 'Access role in Reports.',
      reasons: ['Assigned as Direct'],
  ...
  it('keeps scoped and duplicate active assignments uniquely keyed across collection changes', () => {
