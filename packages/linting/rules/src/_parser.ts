...
const tsWasmPath = _wasmDir
  ? join(_wasmDir, 'tree-sitter-typescript.wasm')
  : _require.resolve('tree-sitter-typescript/tree-sitter-typescript.wasm');
const tsLanguage = await Language.load(await readFile(tsWasmPath));

/**
 * Shared tree-sitter parser pre-loaded with the TypeScript grammar.
 *
