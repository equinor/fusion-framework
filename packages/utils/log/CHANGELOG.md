...
- d333151: Internal: publish every package on the `next` pre-release tag so the whole framework can be installed as a coherent set.
  ...
  ```
- 18ee1cb: Guard the `process.env` access used to detect the current log level so it no longer throws in environments without a Node-style `process` global (e.g. Vitest Browser Mode).
...
- 2836e0b: Guard the `process.env` access used to detect the current log level so it no longer throws in environments without a Node-style `process` global (e.g. Vitest Browser Mode).
...
- 80c3e4a: Internal: Add intent comments for switch/if control flow and iterator/pipe usage, remove a stale TODO, and suppress `single-export-per-file` for the deliberately co-located `LogLevel` enum and `defaultLogLevel` constant, resolving all `fusion-lint` warnings in this package.
