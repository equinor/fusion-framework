...
import { expectObservable } from './helpers/expect-observable.js';
...
describe('StateProvider', () => {
  ...
  describe('getAllItems', () => {
    it('should retrieve all items', async () => {
      ...
      const spy = vi.spyOn(storage, 'allItems');

      const result = await provider.getAllItems();

      expect(result.items).toHaveLength(2);
      expect(result.items).toContainEqual({ key: 'foo', value: 1 });
      expect(result.items).toContainEqual({ key: 'bar', value: 2 });
    ...
    it('should retrieve items with options', async () => {
      // Use helper to generate 12 items
      storage.generateItems(12);

      const options = { limit: 5, skip: 2 };
      const spy = vi.spyOn(storage, 'allItems');

      const result = await provider.getAllItems(options);

      expect(result.items).toHaveLength(5); // Should respect the limit
      expect(result.total_count).toBe(12); // Total count should still be 12
      expect(result.offset).toBe(2); // Should respect the skip
      expect(spy).toHaveBeenCalledWith(options);
    });

    it('should retrieve items with skip only (no limit)', async () => {
      storage.generateItems(8);

      const options = { skip: 3 };

      const result = await provider.getAllItems(options);

      expect(result.items).toHaveLength(5); // Should return remaining items after skip
