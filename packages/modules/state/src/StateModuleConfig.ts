...
export const IStorageSchema = z.object({
  ...
  on: z
    .function()
    .describe(
      'Function to register event listeners. Signature: on(type: StorageChangeEventType, callback: StorageChangeEventHandler) => VoidFunction',
    ),

  onError: z
    .function()
    .describe(
      'Function to register error listeners. Signature: onError(callback: StorageErrorHandler) => VoidFunction',
    ),
