...
- 18ee1cb: Add `./mock` and `./mock/fixtures` subpaths for seeding context in tests without a real context API.
...
- d333151: Internal: publish every package on the `next` pre-release tag so the whole framework can be installed as a coherent set.
  ...
  ```
- 18ee1cb: Declare `@equinor/fusion-framework-module-telemetry` as an optional peer dependency.
  ...
  import at runtime.
- 18ee1cb: Restructure documentation so the README is an entry point rather than a manual, matching the convention already used by `@equinor/fusion-framework-module` and `@equinor/fusion-framework-module-http`.
