...
export function enableLegacyAppNavigationFix(deps: LegacyAppNavigationFixDeps): VoidFunction {
  ...
  return event.addEventListener('onContextNavigationHandlerNavigated', (e) => {
    ...
    if (appNavigation.version.major < LEGACY_NAVIGATION_VERSION) {
      console.warn(
        `[ContextNavigationHandler] App [${appKey}] uses navigation v${appNavigation.version.major} (< ${LEGACY_NAVIGATION_VERSION}). Resetting app router to keep it in sync.`,
      );
