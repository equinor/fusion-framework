...
- 60f34f8: `MsalMockConfigurator` gains a `setToken(token, skipResolve?)` method. When set, `MsalMockClient`
...
- 18ee1cb: Move the MSAL configuration schema into `MsalConfig.schema.ts` and add `MsalConfigExtension`, an extension point for variants of this module.
...
- 18ee1cb: Extract client construction from `MsalConfigurator._processConfig` into overridable seams.
...
- 18ee1cb: Give `MsalMockClient` a real account cache, so its account APIs agree with each other and with MSAL.
  ...
  ```
- 18ee1cb: Added a `./mock` entry point so applications can run against the auth module without credentials or network access.
...
- 18ee1cb: Add `setAccount` to `MsalMockConfigurator`, so a test declares the signed-in user on the builder instead of baking it into a client.
...
- 18ee1cb: Remove a stray `console.log` left in `resolveVersion`'s minor-version-mismatch warning path.
