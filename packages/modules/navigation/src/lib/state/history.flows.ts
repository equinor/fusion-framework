...
import type { Location, LocationState, HistoryStack, Action } from '../types';
  ...
  (action$: Observable<Actions>) => {
    ...
    return action$.pipe(
      ...
      map((action) => {
        ...
        const state = payload.options?.state;
        ...
        switch (type) {
          ...
          case 'REPLACE':
            ...
            break;
          default:
            return navigate.failure(new Error(`Invalid action: ${type}`));
        }
    ...
    return action$.pipe(
      ...
      map((action) => {
        switch (action.type) {
          case navigate.type:
            return {
              type: action.payload.options.action as Action,
              action: action,
            };
          case go.type:
          case pop.type:
