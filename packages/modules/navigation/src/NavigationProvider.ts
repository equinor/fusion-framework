  ...
  implements INavigationProvider
{
  ...
  constructor(args: BaseModuleProviderCtorArgs<INavigationConfigurator>) {
    ...
    // Normalize basename to never end with a trailing slash.
    // Path math in _localizePath and _isWithinBasenameScope relies on a
    // consistent no-trailing-slash form (e.g. "/apps/my-app", not "/apps/my-app/").
