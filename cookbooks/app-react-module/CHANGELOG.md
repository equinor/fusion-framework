...
- 18ee1cb: Internal: add Vitest coverage for the `App` component, covering the demo module's resolved `foo`/`bar` values and overriding `configure` to seed a different demo configuration; wires the cookbook into the root Vitest project list via `vitest.config.ts`.
...
- 2836e0b: Internal: add Vitest coverage for the `App` component, covering the demo module's resolved `foo`/`bar` values and overriding `configure` to seed a different demo configuration; wires the cookbook into the root Vitest project list via `vitest.config.ts`.
...
- 80c3e4a: Internal: resolve `fusion-lint` warnings (`require-intent-comment`, `require-tsdoc`) across `App.tsx` and the `modules/demo` configurator/provider. No behavior change.
