...
- f663b46: Internal: add Vitest coverage for the `App` component, covering the demo module's resolved `foo`/`bar` values and overriding `configure` to seed a different demo configuration; wires the cookbook into the root Vitest project list via `vitest.config.ts`.
