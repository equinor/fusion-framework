...
testWithRouter('renders category options and hides "Clear Filters" when using defaults', async ({ render }) => {
  const { getByRole, unmount } = await render(
    <ProductFilters categories={['electronics', 'furniture']} filter={null} sort="name-asc" inStock={false} />,
  );
...
testWithRouter('shows "Clear Filters" when a non-default filter is active', async ({ render }) => {
  const { getByRole, unmount } = await render(
    <ProductFilters categories={['electronics']} filter="electronics" sort="name-asc" inStock={false} />,
  );
