...
- f663b46: Add an OpenAPI-mock–backed test fixture (`testWithApiMock`) for full-router integration tests, so route/loader tests get deterministic, schema-shaped mock API responses instead of hand-written fixtures.
...
- f663b46: Tests no longer read or seed `window.location`/`window.history`. Now that
...
- f663b46: Internal: update the test suite for `@equinor/fusion-framework-vitest-plugin-react-app`'s
...
- 80c3e4a: Internal: resolve `fusion-lint` warnings (`require-intent-comment`, `require-tsdoc`) across the route pages, including `ProductPage.tsx`, `ProductsPage.tsx`, `UserPage.tsx`, and `UsersPage.tsx`. No behavior change.
